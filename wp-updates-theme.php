...
if( !class_exists('WPUpdatesTechNewsUpdater') ) {
    class WPUpdatesTechNewsUpdater {
    
		var $api_url;		
		var $theme_slug;
		function TechNews_clear_update_transient() {
		...
		function __construct( $api_url,  $theme_slug ) {
			...
			add_filter( 'pre_set_site_transient_update_themes', array(&$this, 'TechNews_check_for_update_') );
			add_action( 'after_theme_row_'.$theme_name, array(&$this, 'TechNews_child_theme_row') );
			...
			if(!strstr($_SERVER['REQUEST_URI'],'plugin-install.php') && !strstr($_SERVER['REQUEST_URI'],'update.php'))
			{
				...
				add_action( 'load-update-core.php', array(&$this,'TechNews_clear_update_transient') );
				add_action( 'load-themes.php', array(&$this, 'TechNews_clear_update_transient') );
				if(!strstr($_SERVER['REQUEST_URI'],'/network/')){
					add_action( 'admin_notices', array(&$this, 'TechNews_child_update_nag') );
				}
				delete_transient( $theme_name.'-update' );
		...
		function TechNews_child_update_nag($transient) {
			...
			{	
				echo '<div id="update-nag">';
				 $new_version = version_compare($theme_version, $remote_version, '<') ? __('There is a new version of TechNews available.', T_DOMAIN) .' <a class="thickbox" title="TechNews Forms" href="http://templatic.com/members/mydownloads/TechNews/theme/change_log.txt">'. sprintf(__('View version %s Details', T_DOMAIN), $remote_version) . '</a>. ' : '';		  			
			          $theme_name = basename(get_stylesheet_directory_uri());
					...
					$ajax_url=site_url('/wp-admin/admin.php?page=TechNews_tmpl_theme_update');
					$download= wp_nonce_url( self_admin_url('update.php?action=upgrade-theme&theme=').$theme_name, 'upgrade-theme_' . $theme_name);
					echo '</tr><tr class="plugin-update-tr"><td colspan="3" class="plugin-update"><div class="update-message">' . $new_version . __( 'or <a href="'.$ajax_url.'" title="TechNews Update">update now</a>.', T_DOMAIN) .'</div></td>';
				echo '</div>';
			}
		...
		function TechNews_check_for_update_( $transient ) {
			global $theme_response,$wp_version;
			...
			$request_string = $this->TechNews_prepare_request( 'templatic_theme_update', $request_args );
		 ...
		 */
		function TechNews_child_theme_row()
		{
			global $theme_response,$wp_version;			
		...
		function TechNews_prepare_request( $action, $args ) {
			global $wp_version;
